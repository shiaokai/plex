...
for k=1:length(cfg.ch)
  if(cfg.ch(k)=='_'), continue; end
  I=zeros(sz,sz,3,n,'uint8'); k0=1;
  fInfo=zeros(n,3);
  for i=1:n % n synthetic examples per character
    if strcmp(type,'train')    
      [I(:,:,:,k0),fInfo(k0,1),fInfo(k0,2),fInfo(k0,3)]=...
          genChar(cfg.ch(k),sz, [cfg.ch, ' '], [1 500]); k0=k0+1;
    else
      [I(:,:,:,k0),fInfo(k0,1),fInfo(k0,2),fInfo(k0,3)]=...
          genChar(cfg.ch(k),sz, [cfg.ch, ' '], [500 500]); k0=k0+1;        
    end
  end
  y=k*ones(1,n); y=y(:);
  writeAllImgs(I,y,cfg.chC,fullfile(cfg.dPath,'synth_w_fonts',type,'charHard'));
  lbl=cfg.ch(k);
  if(lbl>=97&&lbl<=122), lbl=['-' lbl]; end
  dlmwrite(fullfile(cfg.dPath,'synth_w_fonts',type,'charHard',lbl,'fids.dat'),fInfo);
  clear I;
end
end